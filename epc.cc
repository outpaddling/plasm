...
{
    static boost::regex pattern_reg_direct("[Rr][0-9][0-5]?"),
    ...
    // Register direct
    if ( boost::regex_match(operand, pattern_reg_direct) )
    {
    ...
    // Register indirect
    else if ( boost::regex_match(operand, pattern_reg_indirect) )
    {
	sscanf(operand.c_str(), "(r%d)", &reg_num);
    ...
    // Autoincrement
    else if ( boost::regex_match(operand, pattern_auto_increment) )
    {
	sscanf(operand.c_str(), "(r%d)+", &reg_num);
    ...
    // Autodecrement
    else if ( boost::regex_match(operand, pattern_auto_decrement) )
    {
	sscanf(operand.c_str(), "-(r%d)", &reg_num);
    ...
    // Numeric offset
    else if ( boost::regex_match(operand, pattern_numeric_offset) )
    {
    ...
    // Label_offset
    else if ( boost::regex_match(operand, pattern_label_offset) )
    {
    ...
    // Memory direct
    else if ( boost::regex_match(operand, pattern_mem_direct) )
    {
    ...
    // Memory indirect
    else if ( boost::regex_match(operand, pattern_mem_indirect) )
    {
    ...
    // Address of
    else if ( boost::regex_match(operand, pattern_address_of) )
    {
    ...
    // Immediate
    else if ( boost::regex_match(operand, pattern_immediate_int) )
    {
	modeByte[operandCount] = MODE_AUTO_INCREMENT | 15;
    ...
    // Immediate
    else if ( boost::regex_match(operand, pattern_immediate_float) )
    {
	modeByte[operandCount] = MODE_AUTO_INCREMENT | 15;
	statement::add_to_machineCodeSize(1 + get_dataSize());
	statement::add_to_machineCodeCols(3 + get_dataSize() * 2 + 1);
    ...
    // Immediate address
    else if ( boost::regex_match(operand, pattern_immediate_address) )
    {
	modeByte[operandCount] = MODE_MEM_DIRECT;
	statement::add_to_machineCodeSize(1 + 4);
	statement::add_to_machineCodeCols(3 + 9);
